...
HELP = "Serve the patch preview and feedback endpoint."

#: repo_root/preview (this module lives in repo_root/src/cli/).
PREVIEW_DIR = Path(__file__).resolve().parents[2] / "preview"
CONTENT_TYPES = {
...
def create_server(
    ...
    if feedback_file:
        ...
        feedback_path = (
            load_project_paths(discovered)["feedback_path"]
            if discovered
            else Path(__file__).resolve().parents[2] / "data" / "feedback.jsonl"
        )
...
def build_parser(add_help: bool = True) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Serve the Quill preview and collect Knowledge CI feedback.", add_help=add_help
    )
